...
export const register = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    // For Job Seeker, ensure job niches are provided
    ...
    // Handle resume upload using Cloudinary
    if (req.files && req.files.resume) {
      ...
      if (resume) {
        try {
          ...
          // Store resume information in user data
          userData.resume = {
...
export const login = catchAsyncErrors(async(req,res,next) => {
  ...
  if(!role || !email || !password){
    return next(new ErrorHandler("Email, password and rol are required.", 400)
  );
  }
